/* NOTE: This solution is written for a sqlite database with UTF-8 Encoding */
#include <iostream>
#include <omp.h>
#include <sqlite3.h>
#include <string>
#include <vector>

#include "pfaai/data.hpp"
#include "pfaai/runner.hpp"

#define DEFAULT_PROTEIN_SET                                                    \
    {                                                                          \
        "pf00411.19", "pf00237.19", "pf01016.19", "pf02033.18", "pf00347.23",  \
            "pf00119.20", "pf00297.22", "pf02601.15", "pf00318.20",            \
            "pf02367.17", "pf00825.18", "pf02410.15", "pf00406.22",            \
            "pf00380.19", "pf00213.18", "pf05221.17", "pf00252.18",            \
            "pf00177.21", "pf00709.21", "pf00312.22", "pf01192.22",            \
            "pf06026.14", "pf01649.18", "pf00572.18", "pf00338.22",            \
            "pf01142.18", "pf01746.21", "pf01632.19", "pf17136.4",             \
            "pf00164.25", "pf01808.18", "pf00750.19", "pf00889.19",            \
            "pf01196.19", "pf01250.17", "pf00162.19", "pf01725.16",            \
            "pf01668.18", "pf00749.21", "pf00238.19", "pf02565.15",            \
            "pf01715.17", "pf00203.21", "pf00828.19", "pf00573.22",            \
            "pf00121.18", "pf13393.6", "pf00276.20", "pf01139.17",             \
            "pf00886.19", "pf00189.20", "pf01176.19", "pf00687.21",            \
            "pf01025.19", "pf03948.14", "pf00829.21", "pf01195.19",            \
            "pf03840.14", "pf00231.19", "pf01351.18", "pf01264.21",            \
            "pf01193.24", "pf00410.19", "pf00584.20", "pf01245.20",            \
            "pf02130.17", "pf02699.15", "pf01765.19", "pf01783.23",            \
            "pf00281.19", "pf00416.22", "pf00366.20", "pf00344.20",            \
            "pf00831.23", "pf00334.19", "pf00830.19", "pf00861.22",            \
            "pf00453.18", "pf00181.23", "pf03652.15"                           \
    }

int parallelfastaai(const std::string pathToDatabase) {
    std::vector<std::string> proteinSet = DEFAULT_PROTEIN_SET;
    ParFAAIData<int> pfaaiData(pathToDatabase);
    auto errorCode = pfaaiData.validateDBOpen();
    if (errorCode != PFAAI_OK) {
        return errorCode;
    }
    // PHASE 1: Construction of the data structures
    PFAAI_ERROR_CODE pfErrorCode = pfaaiData.construct();
    if (errorCode != PFAAI_OK) {
        return pfErrorCode;
    }
    // Run parallel Fast AAI algorithm
    ParFAAIRunner<int> pfaaiRunner(pfaaiData);
    pfaaiRunner.run();
    return PFAAI_OK;
}

int main(int argc, char* argv[]) {
    if (argc < 2) {
        std::cout << "Missing Database location " << std::endl;
        std::cout << "Usage: " << argv[0] << " Database Location " << std::endl;
        return 1;
    }
    // const std::string pathToDatabse = "modified_xantho_fastaai2.db";
    const std::string pathToDatabase = argv[1];
    return parallelfastaai(pathToDatabase);
}
